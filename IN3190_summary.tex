...
\begin{itemize}
    \item Can be designed with exact linear phase, which makes h(n) zero points symmetric about $|z| = 1$
...
\begin{itemize}
    \item In IIR filter design, the possibility of converting a analog filter to a digital filter gives them several advantages:
    \begin{itemize}
        \item Analog approximation techniques are highly advanced.
        \item They usually yield closed-form solutions.
        \item Extensive tables are available for analog filter design.
        \item Many application require digital simulation of analog system.
    \end{itemize}
    \item IIR filters can be designed to meet the same specifications as FIR filters with a lower order.
\end{itemize}
...
FIR filter design is often based on a approximation of the a specified desired magnitude response. The design of a FIR filter of order $M-1$ can be accomplished by finding either the length $M$ impulse response samples in $\{ h(n) \}$ or the $M$ samples of the frequency response in $\{ H(e^{j\omega}) \}$.
\begin{itemize}
    \item \textbf{Window method:} Multiply the ideal impulse response with a window function to get the desired impulse response.
    \item \textbf{Frequency sampling method:} Sample the frequeny response (DTFT) of the desired filter by assuming it to be the DFT. Then use the inverse DFT to get the desired coefficients. This can work well in the points sampled, but between the points the frequency response is not controlled.
